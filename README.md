...
聊天查看器顶部提供 **管理与导出** 入口，可直接进入 `/panel`。入口同时显示采集状态：

- 采集进行中时显示“正在采集…”并以黄色状态点提示
- 采集失败时显示“采集失败 · 查看”，点击后可进入控制面板查看日志
- 其他状态显示“管理与导出”；查看器每 5 秒刷新一次状态

查看器与控制面板共用同一套访问密码和登录令牌。从任一页面完成密码验证后，切换到另一页面时
无需重复登录；下载导出文件时也会复用相同的认证 Cookie。

> **实现背景：** 项目原先已经具备控制面板、`/panel/api/status` 状态接口、密码认证及
> Cookie 导入等后端和面板功能，但聊天查看器没有通往控制面板的入口，也没有消费采集状态；
> 两个前端还分别使用 `authToken` 与 `panel_token` 保存令牌。因此原有能力并非缺失，而是
> 查看器与控制面板之间的集成尚未完成。本次修改补齐了入口、状态轮询和认证同步。
